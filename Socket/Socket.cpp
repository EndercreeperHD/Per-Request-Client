...
Socket::Socket(std::string host, int port)
{
    ...
    if (ok != 0) {
        ...
        // Kein return; das Objekt existiert, aber connect() wird dann fehlschlagen
    }
...
bool Socket::connect()
{
    ...
    if (rc == SOCKET_ERROR) {
        // Fuer Schueler gut sichtbar:
        // Typisch: Server laeuft noch nicht -> man kann spaeter erneut probieren
        return false;
    }
    return true;
}
...
void Socket::write(int b)
{
    ...
    // Hinweis fuer die Schule: Sender und Empfaenger muessen dasselbe Format erwarten.
...
void Socket::write(char* b, int len)
{
    ...
    // Das Original hat dafuer strcat_s(b, ..., "\n") benutzt (riskant, da Puffer veraendert).
    // Didaktisch sauberer: Wir veraendern den Eingabepuffer NICHT,
    ...
    // (kein size_t, damit es konsistent bleibt)
...
int Socket::read()
{
    ...
    // Einfache Schleife bis 4 Bytes da sind (didaktisch: blockierend, klar vorhersehbar)
    while (got < need) {
...
int Socket::read(char* b, int len)
{
    ...
    if (r <= 0) {
        // Fuer Schueler: r==0 => Gegenstelle hat ordentlich geschlossen
        // r<0 => Fehler
        return 0;
    }
    return r;
...
{
    // Liest Zeichen bis Newline '\n' (blockierend, gut fuer Konsolen-Demos)
    char buffIn[256];
    int pos = -1;
